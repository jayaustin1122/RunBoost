...
import com.example.athlitecsapp.databinding.FragmentEightListsBinding
...
class EightListsFragment : Fragment() {
    private lateinit var binding: FragmentEightListsBinding
    ...
    override fun onCreateView(
        ...
        binding = FragmentEightListsBinding.inflate(layoutInflater)
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        currentPosition = savedInstanceState?.getInt("CURRENT_VIDEO_INDEX1") ?: getCurrentPositionFromPrefs()
        ...
        binding.btnSkip.setOnClickListener {
            try {
                day++
                signUpviewModel.updateLevel2(day)
                findNavController().navigateUp()
            } catch (e: Exception) {
    ...
    private fun saveCurrentPositionToPrefs() {
        val sharedPreferences = requireContext().getSharedPreferences("VideoPrefs1", Context.MODE_PRIVATE)
        val editor = sharedPreferences.edit()
        editor.putInt("CURRENT_VIDEO_INDEX1", currentPosition)
        editor.apply() // Save the value asynchronously
    }
    private fun getCurrentPositionFromPrefs(): Int {
        val sharedPreferences = requireContext().getSharedPreferences("VideoPrefs1", Context.MODE_PRIVATE)
        return sharedPreferences.getInt("CURRENT_VIDEO_INDEX1", 0)
    }
