...
import com.example.athlitecsapp.databinding.DialogBinding
import com.example.athlitecsapp.databinding.FragmentTake100Binding
...
class Take100Fragment : Fragment() {
    private lateinit var binding: FragmentTake100Binding
    ...
    override fun onCreateView(
        ...
        binding = FragmentTake100Binding.inflate(layoutInflater)
        return binding.root
    }
    ...
    fun getDescriptionByLevel2(level: String): String {
        return when (level) {
            ...
            "EXPERT" -> "EXPERT, 6 exercises, 2 sets x 20 counts each"
            else -> "UNKNOWN LEVEL"
        }
    }
    ...
    fun getDescriptionByLevel3(level: String): String {
        return when (level) {
            ...
            "LEVEL3" -> "LEVEL3, 7 exercises, 3 sets x 25 counts each" // Add the new LEVEL3 description
            else -> "UNKNOWN LEVEL"
        }
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        currentPosition = savedInstanceState?.getInt("CURRENT_VIDEO_INDEX") ?: getCurrentPositionFromPrefs()
        binding.btnSkip.setOnClickListener {
            try {
                day++
                signUpviewModel.updateLevel1(day)
        ...
        getWorksOut(day) { workoutDescription ->
            videoList.addAll(
                listOf(
    ...
    private fun getWorksOut(day: Int?, callback: (String) -> Unit) {
        if (day == null) {
        ...
        lifecycleScope.launch {
            viewModel.getWorkoutsById(day) { workout ->
                val workoutDescription = buildString {
                    append(workout?.description ?: "No description available for this workout.")
    ...
    override fun onSaveInstanceState(outState: Bundle) {
        ...
        outState.putInt("CURRENT_VIDEO_INDEX", currentPosition)
    }
    private fun saveCurrentPositionToPrefs() {
        val sharedPreferences = requireContext().getSharedPreferences("VideoPrefs", Context.MODE_PRIVATE)
        val editor = sharedPreferences.edit()
        editor.putInt("CURRENT_VIDEO_INDEX", currentPosition)
        editor.apply() // Save the value asynchronously
    }
    private fun getCurrentPositionFromPrefs(): Int {
        val sharedPreferences = requireContext().getSharedPreferences("VideoPrefs", Context.MODE_PRIVATE)
        return sharedPreferences.getInt("CURRENT_VIDEO_INDEX", 0)
    }
