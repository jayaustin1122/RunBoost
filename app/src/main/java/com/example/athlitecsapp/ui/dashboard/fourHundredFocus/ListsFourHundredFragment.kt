package com.example.athlitecsapp.ui.dashboard.fourHundredFocus
...
import com.example.athlitecsapp.databinding.FragmentListsFourHundredBinding
...
class ListsFourHundredFragment : Fragment() {
    private lateinit var binding : FragmentListsFourHundredBinding
    ...
    override fun onCreateView(
        ...
        binding = FragmentListsFourHundredBinding.inflate(layoutInflater)
        // Inflate the layout for this fragment
        return binding.root
    }
    override fun onSaveInstanceState(outState: Bundle) {
        super.onSaveInstanceState(outState)
        outState.putInt("CURRENT_VIDEO_INDEX4", currentPosition)
    }
    private fun saveCurrentPositionToPrefs() {
        val sharedPreferences = requireContext().getSharedPreferences("VideoPrefs4", Context.MODE_PRIVATE)
        val editor = sharedPreferences.edit()
        editor.putInt("CURRENT_VIDEO_INDEX4", currentPosition)
        editor.apply() // Save the value asynchronously
    }
    private fun getCurrentPositionFromPrefs(): Int {
        val sharedPreferences = requireContext().getSharedPreferences("VideoPrefs4", Context.MODE_PRIVATE)
        return sharedPreferences.getInt("CURRENT_VIDEO_INDEX4", 0)
    }
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        super.onViewCreated(view, savedInstanceState)
        currentPosition = savedInstanceState?.getInt("CURRENT_VIDEO_INDEX4") ?: getCurrentPositionFromPrefs()
        binding.btnSkip.setOnClickListener {
            try {
                day++
                signUpviewModel.updateLevel6(day)
