...
class TakeFourHundredFragment : Fragment() {
    private val viewModel: SignUpModel by viewModels { SignUpFactory(requireContext()) }
    private lateinit var binding : FragmentTakeFourHundredBinding
    override fun onCreateView(
        inflater: LayoutInflater, container: ViewGroup?,
        savedInstanceState: Bundle?
    ): View? {
        binding = FragmentTakeFourHundredBinding.inflate(layoutInflater)
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        viewModel.statusLiveData.observe(viewLifecycleOwner) { status ->
            if (status != null) {
                ...
                val progress = status.level6DayProgress
                    ...
                    DialogUtils.showWarningMessage(
                        ...
                        confirmListener = SweetAlertDialog.OnSweetClickListener { dialog ->
                            val bundle = Bundle().apply {
                                putInt("day", status.level6DayProgress)
