...
import com.example.athlitecsapp.databinding.FragmentListsOneFiveBinding
...
class ListsOneFiveFragment : Fragment() {
    private lateinit var binding : FragmentListsOneFiveBinding
    ...
    override fun onCreateView(
        ...
        binding = FragmentListsOneFiveBinding.inflate(layoutInflater)
        // Inflate the layout for this fragment
        return binding.root
    }
    override fun onSaveInstanceState(outState: Bundle) {
        super.onSaveInstanceState(outState)
        outState.putInt("CURRENT_VIDEO_INDEX5", currentPosition)
    }
    private fun saveCurrentPositionToPrefs() {
        val sharedPreferences = requireContext().getSharedPreferences("VideoPrefs5", Context.MODE_PRIVATE)
        val editor = sharedPreferences.edit()
        editor.putInt("CURRENT_VIDEO_INDEX5", currentPosition)
        editor.apply() // Save the value asynchronously
    }
    private fun getCurrentPositionFromPrefs(): Int {
        val sharedPreferences = requireContext().getSharedPreferences("VideoPrefs5", Context.MODE_PRIVATE)
        return sharedPreferences.getInt("CURRENT_VIDEO_INDEX5", 0)
    }
